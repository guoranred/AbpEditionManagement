    ...
    public class ClientDemoService : ITransientDependency
    {
        ...
        private async Task TestWithHttpClientAndIdentityModelAuthenticationServiceAsync()
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                var url = Configuration["RemoteServices:EditionManagement:BaseUrl"] +
                          "api/EditionManagement/sample/authorized";
        ...
        private async Task TestAllManuallyAsync()
        {
            ...
            var disco = await client.GetDiscoveryDocumentAsync(Configuration["IdentityClients:Default:Authority"]);
            if (disco.IsError)
            {
            ...
            var tokenResponse = await client.RequestPasswordTokenAsync(new PasswordTokenRequest
            {
                ...
                ClientId = Configuration["IdentityClients:Default:ClientId"],
                ClientSecret = Configuration["IdentityClients:Default:ClientSecret"],
                UserName = Configuration["IdentityClients:Default:UserName"],
                Password = Configuration["IdentityClients:Default:UserPassword"],
                Scope = Configuration["IdentityClients:Default:Scope"]
            });
